...
function header(time) {
    ...
    if(？1 && currentTime <= time + validityPeriod){ // 未添加导航,并且在有效期内
        var title = $('.O-j-k>.O-j');
        if(title.length === 0){//判断导航是否已经加载出来
            setTimeout(function (){header(time)}, 1000);
        }else{
            $('.O-j>.O-j-p-J').remove();
            $('.O-j-gb>.A8AIPb>.PNF6le').html('Chrome插件网');
			//if($('#my_navigate').length === 0){//判断header是否已经添加
...
function sidebar(time){
    // 判断sidebar是否已经修改
    var currentTime = new Date().getTime();
    if(？2 && currentTime <= time + validityPeriod){//未修改,并且在有效期内
...
function footer(time){
    // 判断尾部是否已经添加
    var currentTime = new Date().getTime();
    if(？3 && currentTime <= time + validityPeriod){// 未添加
        var footer = $('.F-ia-k');
        if(footer.length === 0){
            setTimeout(function (){footer(time)},1000);
        }else{
			//if($('#my_footer').length === 0){
...
function detailFooter(time){
    // 判断尾部是否已经添加
    var currentTime = new Date().getTime();
    if(？4 && currentTime <= time + validityPeriod){// 未添加
...
function button(time){
    //判断button是否添加
    var currentTime = new Date().getTime();
    if(？5 && currentTime <= time + validityPeriod) {
